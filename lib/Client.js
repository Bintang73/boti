      ...
      */
       ...
       */
       async sendImage(from, content, caption, id) {
         if (content.url && /((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w-_]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?)/.test(content.url)) {
          let generateThumb = await axios.get(content.url, { responseType: "arraybuffer"} )
         let captJid = caption ? [...caption.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net') : ""
         return this.sendMessage(from, generateThumb.data, MessageType.image, { quoted: id, caption: caption, mimetype: Mimetype.jpeg, thumbnail: generateThumb.data.toString("base64"), contextInfo: { mentionedJid: captJid } } )
         } else {
         let captJid = caption ? [...caption.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net') : ""
         return this.sendMessage(from, content, MessageType.image, { quoted: id, caption: caption, mimetype: Mimetype.jpeg, thumbnail: content.toString("base64"), contextInfo: { mentionedJid: captJid } } )
         }
       }
      /**
